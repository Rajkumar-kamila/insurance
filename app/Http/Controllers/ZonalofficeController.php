...
class ZonalofficeController extends Controller
{
    ...
    {
        $fetchdata = LoginModel::where('flag', '0')->where('admin_role', 'zonal_office')->get();
        $fetchsidebar = SidebarModel::where('sidebar_name','!=','admin')->where('sidebar_name','!=','company')->where('sidebar_name','!=','head office')->where('sidebar_name','!=','regional office')->where('sidebar_name','!=','zonal office')->get();
        return view('staff.zonal_office.add',compact('fetchdata','fetchsidebar'));
    }
                ...
                if(!empty($existingEmail)){
                    ...
                    if($request->password == $request->c_password){
                        ...
                        if(!empty($inserted)){
                            ...
                            return response()->json(['code' => 200, 'message' => 'Zonal Office Added Successfully']);
                        }else{
